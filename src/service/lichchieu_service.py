...
from repository.lichchieu_repository import LichChieuRepository
from util.Lichchieu import LichchieuUtil
from util.response import Res
from util import time
from util.genarate import gen_time, gen_number
...
class LichChieuService:
    ...
    def searchTimKiem(self, date, selected_room):
        start_timestamp = time.convertTimeToTimestamp(f"{date}", "%m/%d/%Y")
        ...
        for item in data:
            nameMovie = item[2]
            room = item[1]
            date = time.convertTimeStampToString(item[0], "%m/%d/%Y")
            day = time.convertTimeStampToString(item[0], "%m")
            month = time.convertTimeStampToString(item[0], "%d")
    ...
    def add_lichchieu(self, movie, room: str, datetime):
        ...
        playAt = int(time.convertTimeToTimestamp(datetime, "%H:%M %m/%d/%Y"))
